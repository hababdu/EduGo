...
describe('seededShuffle (19-band: random answer order)', () => {
    ...
    it('bir xil seed + questionId doim bir xil tartibni beradi (determinizm)', () => {
        const first = (0, seeded_shuffle_util_1.seededShuffle)(items, 12345, 'question-1');
        const second = (0, seeded_shuffle_util_1.seededShuffle)(items, 12345, 'question-1');
    ...
    it('har bir chaqiriqda ORIGINAL massivni o\'zgartirmaydi (immutability)', () => {
        ...
        (0, seeded_shuffle_util_1.seededShuffle)(items, 999, 'question-2');
    ...
    it('turli questionId uchun (bir xil seed bilan) ehtimol boshqacha tartib beradi', () => {
        const forQ1 = (0, seeded_shuffle_util_1.seededShuffle)(items, 42, 'question-1');
        const forQ2 = (0, seeded_shuffle_util_1.seededShuffle)(items, 42, 'question-2');
        expect(forQ1.map((i) => i.id)).not.toEqual(forQ2.map((i) => i.id));
    });
    it('barcha elementlarni saqlab qoladi (yo\'qotmaydi, takrorlamaydi)', () => {
        const result = (0, seeded_shuffle_util_1.seededShuffle)(items, 7, 'question-3');
        expect(result.map((i) => i.id).sort()).toEqual(items.map((i) => i.id).sort());
    });
});
//# sourceMappingURL=seeded-shuffle.util.spec.js.map